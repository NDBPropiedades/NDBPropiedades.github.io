...
function aplicarFiltros(lista) {
  ...
  if (q) {
    arr = arr.filter((p) => {
      ...
      const campos = [
        ...
        p.type?.name,
        (() => {
          const op0 = p.operations?.[0];
          const po = op0?.prices?.[0];
          return po ? formatearPrecioTokko(po) : "";
        })(),
      ];
  ...
  if (orden) {
    arr.sort((a, b) => {
      const pa = precioNumerico(a);
      const pb = precioNumerico(b);
...
function precioNumerico(p) {
  const op0 = p.operations?.[0];
  const pr = op0?.prices?.[0];
  return {
    currency: (pr?.currency || "").toUpperCase(),
    value: Number(pr?.price || 0),
  };
}
...
function formatearPrecioTokko(priceObj){
  if (!priceObj || priceObj.price == null) return "Consultar";
  ...
  return `${symbol}${Number.isFinite(n) ? n.toLocaleString("es-AR") : priceObj.price}`;
}
...
function precioDeOperacion(p, opS){
  ...
  const priceObj = op?.prices?.[0];          // { currency:"ARS", price:550000, ... }
...
function construirControlesPaginacion(total, actual, porPagina) {
  ...
  paginacionNav.appendChild(
    btn("»", Math.min(totalPaginas, actual + 1), actual === totalPaginas),
  );
}
  ...
  items.forEach((p, idx) => {
    const fotos = p.photos?.length
      ? p.photos
          .map(
            (ph) =>
              `<div class="swiper-slide"><img src="${ph.image}" loading="lazy" /></div>`,
          )
          .join("")
    ...
    if (baths > 0)
      iconos += `<span><i class="fas fa-bath"></i> ${baths}</span>`;
    if (toil > 0)
      iconos += `<span><i class="fas fa-toilet"></i> ${toil}</span>`;
    if (surf > 0)
      iconos += `<span><i class="fas fa-ruler-combined"></i> ${surf} m²</span>`;
