  ...
  return fetch(
    ...
    .then((e) => {
      (contenedorCards.innerHTML = ""),
        e.objects.forEach((e, a) => {
          let r = document.createElement("div");
          r.className = "swiper-slide";
          let t = (e.photos || [])
              .map(
                (a) => `
        ...
        `
            ...
            s =
              e.operations?.[0]?.operation_type === "Sale"
                ...
                : "Alquiler",
            i =
              propertyTypeTranslations[e.type?.name] ||
              e.type?.name ||
              "Propiedad",
            o = e.location?.name || "Ubicaci\xf3n no especificada",
        ...
        ${
          d > 0
            ? `<span><i class="fas fa-ruler-combined"></i> ${d} m\xb2</span>`
            : ""
        }
      `;
          (r.innerHTML = `
...
animables.forEach((e) => observer.observe(e));
const pasos = document.querySelectorAll(".step"),
  path = document.getElementById("snake-path"),
  trail = document.getElementById("snake-trail"),
  totalLength = path.getTotalLength(),
  pasosCount = pasos.length,
  stepLength = totalLength / pasosCount;
let pasoActual = 0;
function moverCabeza(e) {
  let a = stepLength * e - 5,
    r = path.getPointAtLength(a),
    t = path.getPointAtLength(a + 5);
  t.x,
    r.x,
    t.y,
    r.y,
    trail.setAttribute("stroke-dashoffset", totalLength - (stepLength * e - 5));
}
function activarPaso() {
  pasos.forEach((e, a) => {
    e.classList.toggle("active", a === pasoActual);
  }),
