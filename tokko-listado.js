...
function poblarTipologiasUnicas(propiedades) {
  ...
  propiedades.forEach((p) => {
    if (p.type?.name) tiposSet.add(p.type.name);
  });
...
function poblarZonasUnicas(propiedades) {
  ...
  propiedades.forEach((p) => {
    if (p.location?.name) zonasSet.add(p.location.name);
  });
    ...
    .then((data) => {
      ...
      if (orden && orden !== "default") {
        propiedades.sort((a, b) => {
          ...
          if (orden === "titulo-asc")
            return (a.publication_title || "").localeCompare(
              b.publication_title || ""
            );
          if (orden === "titulo-desc")
            return (b.publication_title || "").localeCompare(
              a.publication_title || ""
            );
          if (orden === "fecha-asc")
      ...
      propiedades.forEach((prop, index) => {
        ...
        const tipoTransaccion =
          prop.operations?.[0]?.operation_type === "Sale" ? "Venta" : "Alquiler";
        ...
        const tipoPropiedadES =
          propertyTypeTranslations[tipoPropiedadEN] || tipoPropiedadEN;
        ...
        let iconosHTML = "";
        if (dormitorios > 0)
          iconosHTML += `<span><i class="fas fa-bed"></i> ${dormitorios}</span>`;
        if (baños > 0)
          iconosHTML += `<span><i class="fas fa-bath"></i> ${baños}</span>`;
        if (toiletes > 0)
          iconosHTML += `<span><i class="fas fa-toilet"></i> ${toiletes}</span>`;
        if (tamaño > 0)
          iconosHTML += `<span><i class="fas fa-ruler-combined"></i> ${tamaño} m²</span>`;
        ...
        const html = `
          <div class="tokko-card">
            ...
            <div class="info">
              <h3><a href="propiedad.html?id=${
                prop.id
              }">${tipoPropiedadES} en ${zona}</a></h3>
